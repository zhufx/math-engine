...
type defS struct {
	argc int
	fun  func(expr ...ExprAST) decimal.Decimal
}
...
//
// abs(-2) = 2
func defAbs(expr ...ExprAST) decimal.Decimal {
	return (ExprASTResult(expr[0])).Abs()
}

// ceil(4.2) = ceil(4.8) = 5
func defCeil(expr ...ExprAST) decimal.Decimal {
	return ExprASTResult(expr[0]).Ceil()
}

// floor(4.2) = floor(4.8) = 4
func defFloor(expr ...ExprAST) decimal.Decimal {
	return ExprASTResult(expr[0]).Floor()
}

// round(4.2) = 4
// round(4.6) = 5
func defRound(expr ...ExprAST) decimal.Decimal {
	return ExprASTResult(expr[0]).Round(0)
}
...
// max(2, 3) = 3
// max(2, 3, 1) = 3
func defMax(expr ...ExprAST) decimal.Decimal {
	if len(expr) == 0 {
...
// min(2, 3) = 2
// min(2, 3, 1) = 1
func defMin(expr ...ExprAST) decimal.Decimal {
	if len(expr) == 0 {
...
// noerr(1/0) = 0
// noerr(2.5/(1-1)) = 0
func defNoerr(expr ...ExprAST) (r decimal.Decimal) {
	defer func() {
		if e := recover(); e != nil {
			r = decimal.Zero
		}
